...
module.exports.init = async (botModel, bot) => {
  ...
  bot.start(async (ctx) => {
   ...
   let user = await getUser(bot.instanceName, ctx.update.message.from.id);
    ...
    await ctx.reply('Добро пожаловать на платформу развития человека от Института Коллективного Разума. Пройдите все квесты и станьте Новым Человеком.\n\n');
...
async function refreshAction(bot, ctx, user, hostname, balanceId, currentIndex) {
  ...
  async function printQuests(ctx){
    ...
    text += `Интерактивный квест-ретрит состоит из семи зон для раскрытия внутреннего потенциала человека:\n`
    ...
    text += `\nПосле прохождения семи зон, вы поймёте, как просто устроен мир.`
    const buttons = [];
...
Про закаты
Про чудо берега
Как он попав на остров
Не совершил греха

Свой грех опишут люди
Читающие стих
Для этой зоны квеста
Найдя свободный стиль.

Очистить подсознание
Намерен человек.
Хоть в прозе, хоть стихами 
Описывай свой грех
...
Не делая суда.
И чаще улыбайтесь 
Ведь ваша жизнь смешна.

А следущий пришедший
Прочтя греха рассказ,
Запишет откровения
-«его судьбы рассказ»

Страниц не много в книге 
Прочтя её с нуля
Узнаёшь как покается 
Не совершив греха.

«Творенье» вас услышит
Узнав себя в себе.
Добавит место встречи
Идущее к тебе .
