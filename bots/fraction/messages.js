...
async function sendMessageToUser(bot, user, message, extra) {
  try {
    ...
    if ('voice' in message) id = await bot.telegram.sendVoice(user.id, message.voice.file_id, extra);
    ...
    if ('doc' in message) {
      ...
      // eslint-disable-next-line max-len
      id = await bot.telegram.sendVenue(user.id, message.location.latitude, message.location.longitude, message.venue.title, message.venue.address, extra);
