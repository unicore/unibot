...
module.exports.init = async (botModel, bot) => {
  ...
  bot.action('finisheducation', async (ctx) => {
    ...
    await ctx.replyWithHTML('Для того, чтобы совершить вклад в развитие NFT-бизнеса и получать % от его прибыли, вам необходимо обладать цифровой валютой FLOWER.\n\nДля получения FLOWER, отправьте USDT.TRC20 на свой персональный адрес в кошельке. После получения FLOWER, выберите NFT-фракцию бизнеса и совершите свой вклад в неё.\n\nКнопки: \n💎 кошелёк - это ваш лицевой счёт для хранения учётной единицы вкладов [FLOWER].\n✴️ полезные действия - совершайте полезные действия для развития и получайте NFT-фракции.\n❇️ купить NFT - выберите NFT-фракцию бизнеса и совершите свой вклад в развитие бизнеса;\n🛑 продать NFT - верните NFT-фракцию бизнеса в клуб согласно её программе выкупа.\n\nС вопросами принципах работы клуба заходите в наш онлайн-офис: @coop_club', { disable_web_page_preview: true, ...menu });
  ...
  bot.hears('❇️ купить NFT', async (ctx) => {
    const user = await getUser(bot.instanceName, ctx.update.message.from.id);
  ...
  bot.hears('🛑 продать NFT', async (ctx) => {
    const user = await getUser(bot.instanceName, ctx.update.message.from.id);
  ...
  bot.hears('✴️ выбрать действие', async (ctx) => {
    // await checkForExistBCAccount(bot, ctx);
    const user = await getUser(bot.instanceName, ctx.update.message.from.id);
