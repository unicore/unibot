...
module.exports.init = async (botModel, bot) => {
  ...
  bot.start(async (ctx) => {
    ...
    await ctx.reply('Купи фракцию телеграм-канала с доходностью до 100%. Стоимость фракции растёт за счёт продаж рекламы и спроса на фракции у новых фракционеров.',  menu);
  ...
  bot.hears('🌀 рынок фракций', async (ctx) => {
    let user = await getUser(bot.instanceName, ctx.update.message.from.id);
    if (!user) {
  ...
  bot.hears('🌀 мои фракции', async (ctx) => {
