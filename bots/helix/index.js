  ...
  for (const user of users) {
    ...
    if (need_register) {
      ...
      try {
        ...
        if (message.data)
          if (message.data.status === 'ok') console.log('restored account: ', user.eosname, message.data); else
...
module.exports.init = async (botModel, bot) => {
  ...
  bot.start(async (ctx) => {
    ...
    await ctx.reply('Добро Пожаловать в Двойную Спираль! ⚪️🌼⚫️');
  ...
  bot.hears('🌀 касса', async (ctx) => {
    ...
    if (user.is_demo) await printHelixWallet(bot, ctx, user, bot.getEnv().DEMO_HOST); else if (bot.getEnv().MODE === 'community') await printHelixWallet(bot, ctx, user, bot.getEnv().COMMUNITY_HOST);
  });
  ...
  bot.action(/buywith (\w+)/gi, async (ctx) => {
    ...
    if (orders.length > 0) ctx.editMessageText('Выберите заявку и нажмите на неё:', Markup.inlineKeyboard(buttons, { columns: 1 }).resize()); else ctx.editMessageText('На данный момент в системе нет заявок на получение помощи. Возвращайтесь позже.');
  });
