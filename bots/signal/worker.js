...
const {getAllRates} = require('./trading')

const work = async (bot) => {
  console.log('Running worker for', bot.instanceName);

  await Promise.all([
    getAllRates(bot).catch((e) => {
      console.error('error on get signals: ', bot.instanceName)
      console.error(e)
    })
...
const worker = async () => {
  ...
  while (true) {
    ...
    try {
      ...
      const bots = await Bot.find({ isActive: true, mode: 'signal' });
