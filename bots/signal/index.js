...
module.exports.init = async (botModel, bot) => {
  ...
  bot.start(async (ctx) => {
    ...
    if (ctx.update.message.chat.type === 'private') {
      ...
      // await ctx.replyWithHTML(message, clearMenu, { reply_markup: { remove_keyboard: true } });
        ...
        await ctx.replyWithHTML('<b>Добро пожаловать в SIGNALьный бот! </b>\nДля получения сигналов для арбитража введите кодовое слово: ', clearMenu, { reply_markup: { remove_keyboard: true } })
    ...
    if (user) {
      if (ctx.update.message.chat.type !== 'private') { // CATCH MESSAGE ON ANY PUBLIC CHAT WHERE BOT IS ADMIN
        ...
        if (text === 'КОД') {
          ...
          const t = 'Код пулучен. Теперь вы получаете сигналы.';
  ...
  bot.action('cantbuyticket', async (ctx) => {
    ...
    await cantBuyTicket(ctx, bot, user);
