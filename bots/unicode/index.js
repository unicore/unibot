...
const quizDefinition = [
  { message: 'Contacts' },
  { message: 'Давайте познакомимся! Пожалуйста, расскажите кратко о себе и своих навыках в разработке ПО?\n\nОбладая этой информацией, мы сможем создать индивидуальное предложение по обучению для вас 🙏\n\n> введите текст:' },
...
async function nextQuiz(bot, user, ctx) {
  ...
  if (q) {
    ...
    const id = await ctx.reply('Поздравляем! Вы выполнили первое задание при входе в клуб! Нам нужно время, чтобы создать индивидуальное предложение для вас. Оставайтесь на связи!\n\nИ если у вас есть вопросы по работе клуба - просто задавайте их в этом чате (только текст).');
  ...
  bot.start(async (ctx) => {
    ...
    if (ctx.update.message.chat.type === 'private') {
      if (!ctx.update.message.from.is_bot) {
        ...
        const t = 'Добро пожаловать в элитный клуб. 🔯\n\nВсе крупные мировые компании активно внедряют блокчейн, а разработчиков, кто в этом хоть что-то понимает - почти нет, и это тормозит прогресс. 😥\n\nМы предлагаем вам войти в ряды инженеров-исследователей, обладающих навыком запуска корпоративных блокчейнов для сообществ и бизнеса. 📟 \n\nВ нашем клубе вы:\n- Научитесь проектировать и запускать выделенный блокчейн с цифровой экономикой 🔗\n- Научитесь писать смарт-контракты с бизнес-логикой любой сложности 💻\n- Научитесь создавать интерфейсы для работы со своими смарт-контрактами 👨‍🎨\n\nОбучение проходит в формате еженедельных видео-уроков с самостоятельным заданием, за выполнение которого, выдаются ценные NFT. По завершению обучения, вы получите сертификат на блокчейне с подтверждением пройденного пути и возможность трудоустройства в клуб или к партнёрам клуба 🌀';
      ...
      const t = 'Добро пожаловать в клуб блокчейн-разработчиков.\n\n';
